...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  cat <<'EOF'
Usage: scripts/linux-amd64/build-packages.sh [--with-portable]

Use Docker BuildKit and the Debian 12 amd64 baseline to create a .deb package.
On a native x86-64 Docker host, --with-portable also creates an AppImage and
Fedora-compatible RPM. --with-appimage remains accepted as a legacy alias.
Output includes SHA-256 checksums in a new timestamped artifact directory.
EOF
  exit 0
fi
if [[ "${1:-}" == "--with-portable" || "${1:-}" == "--with-appimage" ]]; then
  with_cross_distro=1
  shift
fi
...
bundles="deb"
if [[ "$with_cross_distro" == "1" ]]; then
  docker_arch="$(docker info --format '{{.Architecture}}')"
  if [[ "$docker_arch" != "x86_64" && "$docker_arch" != "amd64" ]]; then
    printf 'AppImage/RPM packaging requires a native x86-64 Docker host; detected %s. Build the .deb here or run --with-portable on an amd64 machine.\n' "$docker_arch" >&2
    exit 1
  fi
...
if [[ "$with_cross_distro" == "1" ]]; then
  ...
  rpm_count="$(find "$output" -maxdepth 1 -type f -name '*.rpm' -print | wc -l | tr -d ' ')"
  [[ "$rpm_count" -eq 1 ]] || {
    printf 'Expected exactly one current RPM artifact; found %s.\n' "$rpm_count" >&2
    exit 1
  }
  rpm_package="$(find "$output" -maxdepth 1 -type f -name '*.rpm' -print -quit)"
  file "$rpm_package" | grep -Eqi 'RPM.*(x86-64|x86_64)' || { printf 'RPM is not x86-64.\n' >&2; exit 1; }
fi
