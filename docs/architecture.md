...
The desktop application maintains a rebuildable SQLite FTS index under its application-data directory, isolated by canonical project path. Project opening reconciles file modification metadata incrementally, saved sheets update only their own index entry, and search reads from FTS. A missing, incompatible, or corrupt index is rebuilt from Markdown; if indexing remains unavailable, the application falls back to bounded direct scans. SQLite is always derived state and never becomes authoritative manuscript storage. Global navigation fans one query out across the separate Inbox and open-project indexes, then merges origin-labelled results in memory. Selecting a result activates its ordinary Markdown folder before opening the sheet; no combined proprietary library database owns the documents. Recent and favorite sheet references are local interface preferences containing paths and summary metadata only; they never create or modify files inside a project.
...
Sheet multi-selection is an explicit, library-scoped interface mode. A batch may include sheets from several folders in the active Inbox or project, but never silently crosses into another source library. Move and Trash operations reuse the same atomic single-sheet commands sequentially, updating local recent and favorite references after each success. If one item fails, already completed files remain valid, unprocessed items stay selected, and the interface reports the exact partial progress instead of pretending the batch was an all-or-nothing transaction. Multi-sheet export assembles only the selected Markdown documents in the visible sort order and remains a one-way product operation.
...
The editor has explicit source, preview, and review boundaries. Write mode remains a native textarea so operating-system spelling, selection, input-method behavior, and the latency-sensitive save path stay simple. Preview mode parses the current in-memory Markdown only when preview is visible, sanitizes the resulting HTML, and renders headings, emphasis, lists, quotations, code, tables, and other presentation structure without modifying the source. Review mode is an optional companion pane backed by a writer-controlled, self-hosted LanguageTool HTTP endpoint. A connection test reads only the server's language list. The app sends the current in-memory sheet only after a separate explicit check action, masks Markdown while retaining source-compatible UTF-16 offsets, and splits long text outside the keystroke path. Public unencrypted endpoints are rejected; private-network HTTP requires a visible acknowledgement. A replacement is applied only while the checked source range still matches, and a failed or unavailable checker never blocks writing or autosave. An empty Inbox or project never mounts an editable surface without an actual sheet.

Document export is a one-way product boundary. The frontend obtains an explicit destination from the native Save dialog and sends an ordered current-sheet, folder, or project assembly to a background Rust worker. One CommonMark event parser strips YAML front matter and produces a small shared manuscript structure. DOCX maps that structure into named WordprocessingML styles; PDF lays it out deterministically on Letter pages with locally embedded Source Serif 4 faces; EPUB maps it to a self-contained EPUB 3 package with reflowable XHTML, navigation, metadata, and CSS. Title pages, section breaks, title/author/language metadata, and reusable frontend presets remain export settings rather than manuscript metadata. Every exporter writes to a temporary file beside the destination, flushes it, and atomically replaces the selected file. Export never reads back into, annotates, or otherwise changes the Markdown source. Images and tables remain later extensions of the shared model.
